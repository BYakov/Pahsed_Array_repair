...
sul_bmk::sul_bmk( device *hw, const char *sul_bmk_config, const char *ram_bmk_config, int modules, int channels ):
sul_base( sul_bmk_config, ram_bmk_config, modules, channels)
{
        cout << "sul_bmk::sul_bmk()" << endl;
...
int sul_bmk::detect()
{
        cout << "sul_bmk::detect_abonents()" << endl;

        int i=0;
        unsigned char   *addresses = new unsigned char[number_of_abonents];
        ...
        int   res = 0;
        for(i=1; i<number_of_abonents+1; i++) {
        ...
        for(i=1; i<=number_of_abonents; i++) {
...
void sul_bmk::info()
{
        ...
        for(i=1; i<abonents.size(); i++) {
                ...
                if(res < 0) {
                        //cout << "sul_bmk::info(): Abonent " << int(address(i)) << ": target_eeprom_crc - error" << endl;
                        --err;
                }
...
int sul_bmk::write_eeprom_block(int zone, int block)
{
        //cout << "sul_bmk::write_eeprom_block()" << endl;
...
int sul_bmk::read_eeprom_block(int zone, int block, unsigned char *buffer, int size)
{
        //cout << "sul_bmk::read_eeprom_block()" << endl;
...
int sul_bmk::write_ram_block(int zone, int block)
{
        //cout << "sul_bmk::write_ram_block()" << endl;
...
int sul_bmk::read_ram_block(int zone, int block, unsigned char *buffer, int size )
{
        //cout << "sul_bmk::read_ram_block()" << endl;
...
bool sul_bmk::is_online(unsigned addr)
{
...
unsigned char sul_bmk::bn_to_addr(unsigned bn)
{
    return bn+1;
}
...
unsigned char sul_bmk::address(unsigned addr)
{
    return addr;
}
...
int sul_bmk::sul_init(int addr)
{
        //cout << "sul_bmk::sul_init()" << endl;
...
int sul_bmk::sul_reset(int addr)
{
        //cout << "sul_bmk::sul_reset()" << endl;
...
//------------------------------------------------------------------------------
/*
int sul_bmk::sul_test_exchange(int addr, unsigned char *buffer, int size)
{
    if(!is_online(addr)) return -1;
...
int sul_bmk::sul_test_channels(int addr)
{
    if(!is_online(addr)) return -1;
...
int sul_bmk::sul_test_calculators(int addr)
{
    if(!is_online(addr)) return -1;
...
int sul_bmk::sul_test_pulses(int addr)
{
    if(!is_online(addr)) return -1;
...
int sul_bmk::sul_beam_position(u16 x, u16 y, u8 zone, ray_type type)
{
        if(!is_any()) return -1;
...
struct sul_state sul_bmk::sul_state(int addr)
{
...
int sul_bmk::sul_flash_erase(int addr)
{
        struct host_dbg_header hdr = {0};
        //cout << "sul_bmk::sul_erase_flash()" << endl;
...
int sul_bmk::sul_flash_load(const char *fname, int addr)
{
        cout << "sul_bmk::sul_load_flash( " <<
        ...
        if(addr) {
            ...
            cout << "sul_bmk::sul_load_flash( " << int(address(addr)) << " )" << endl;
...
u16 sul_bmk::sul_flash_crc(int addr)
{
        //cout << "sul_bmk::sul_flash_crc()" << endl;
...
int sul_bmk::sul_flash_save(const char *file)
{
        //if(file)
...
int sul_bmk::sul_ram_load(const char *fname, int addr)
{
    cout << "sul_bmk::sul_ram_load( " <<
    ...
    if(addr) {
        ...
        cout << "sul_bmk::sul_ram_load( " << int(address(addr)) << " )" << endl;
...
int sul_bmk::sul_ram_save(const char *file)
{
    //if(file)
...
int sul_bmk::sul_ram_erase(int addr)
{
    //cout << "sul_bmk::sul_ram_erase()" << endl;
...
u16 sul_bmk::sul_ram_crc(int addr)
{
    //cout << "sul_bmk::sul_ram_crc()" << endl;

    int res = 0;
    int err = 0;
    u16 *abonent_crc = new u16[number_of_abonents];
    u16 crc = 0;
...
int sul_bmk::sul_set_scale(int addr, unsigned char scale, unsigned char reset_duration, unsigned char pause_duration)
{
...
unsigned char sul_bmk::sul_get_scale(int addr)
{
