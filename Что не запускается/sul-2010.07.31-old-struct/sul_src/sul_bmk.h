...
//---------------------------------------------------------------------------
//!  Класс реализует интерфейс к СУЛ БМК.
/*!
Отвечает за взаимодействие с СУЛ БМК. Реализует все интерфейсные функции,
определенные в классе sul_base.
*/
class sul_bmk : public sul_base
{

private:
        //! Формирователь пакетов для обмена с СУЛ БМК
        packet485	*packet;
...
public:
        ...
        /*!
        \param hw - устройство для обмена с СУЛ БМК
        \param sul_mdo_config - имя файла конфигурации СУЛ
        \param ram_mdo_config - имя файла конфигурации RAM
        */
        sul_bmk( device *hw,
                const char *sul_bmk_config,
                const char *ram_bmk_config,
                int modules, int channels );
