...
int sul_base::sul_phase_shifter_info(unsigned char X, unsigned char Y, struct phase_shifter& shifter)
{
    ...
    if(X >= lines.size() || X < 0) {
        //cout << "Invalid coordinate X: " << int(X) << endl;
        return -1;
    }
    ...
    for(int j=0; j<lines.at(X); j++) {
        if(Y == shifters.at(off + j).y) {
            //cout << "Found Y: " << int(shifters.at(off + j).y) << endl;
            L = j;
            break;
        }
    }
    ...
    if(L < 0) {
        //cout << "Invalid coordinate pair (" << int(X) << ", " << int(Y) << ")" << endl;
        return -1;
    }

    N = L + off;

    if(Y != shifters.at(N).y || N >= shifters.size()) {
        //cout << "Invalid channel number: " << int(N) << endl;
        return -1;
    }
